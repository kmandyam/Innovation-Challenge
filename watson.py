# coding=utf-8
from __future__ import print_function
import json
from watson_developer_cloud import LanguageTranslatorV2

language_translator = LanguageTranslatorV2(
    username='bbff9b6b-0a00-4931-b0bf-86e6fc9d71e4',
    password='4qKI3DFNYZIE')

# create new custom model
# with open('../resources/language_translator_model.tmx', 'rb') as \
#         custom_model:
#     print(json.dumps(language_translator.create_model(
#         base_model_id='en-fr', name='test_glossary',
#         forced_glossary=custom_model), indent=2))

# print(json.dumps(language_translator.list_models(), indent=2))
#
# print(
#     json.dumps(language_translator.get_model('en-es-conversational'),
#                indent=2))

# delete custom model
# print(json.dumps(language_translator.delete_model(
# '13860c86-ec3f-4e60-8cbe-3ef0048f92af'), indent=2))

# print(json.dumps(
#     language_translator.translate('मुझे कुत्ते पसंद है', source='hi',
#                                   target='en'), indent=2,
#     ensure_ascii=False))

# print(json.dumps(
#     language_translator.translate('Hola, cómo estás?', source='es',
#                                   target='en'), indent=2,
#     ensure_ascii=False))

# print(json.dumps(
#     language_translator.translate('I got a good grade in that class',
#                                   model_id='en-es-conversational'),
#     indent=2))

while(True):
    sentence = input('Enter an English Sentence: ')
    ans = language_translator.translate(sentence,
                              model_id='en-es-conversational')
    print("Spanish Translation")
    print(ans["translations"][0]["translation"])

# print(json.dumps(language_translator.identify('你好'), indent=2))
# print(json.dumps(language_translator.identify('मुझे कुत्ते पसंद है'), indent=2))


# print(json.dumps(language_translator.list_identifiable_languages(), indent=2))
